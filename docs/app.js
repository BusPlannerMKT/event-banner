// ── BusPlanner Event Banner Builder — Static/Client-Side Version ──
// No server required. Pexels API called directly from browser.
// Email HTML rendered client-side via renderEmailHTML().

const PEXELS_API_KEY = 'kxEhYr6sTltkZndcCHuo15h2YqMmD4ndTvskPpHPIuU6JiTkganfheKD';
...
// ── Image Search (Direct Pexels API — no server proxy) ──
async function searchImages() {
    ...
    try {
        const resp = await fetch(`https://api.pexels.com/v1/search?query=${encodeURIComponent(query)}&per_page=9&orientation=landscape`, {
            headers: { 'Authorization': PEXELS_API_KEY },
        });

        if (!resp.ok) {
            grid.innerHTML = '<div class="image-grid-loading">Image search unavailable. Try again later.</div>';
            return;
        }
        ...
        grid.innerHTML = data.photos.map(photo => {
            const full = photo.src.large2x || photo.src.large;
            const thumb = photo.src.medium;
            const alt = photo.alt || photo.photographer;
            const photographer = photo.photographer;
            return `<div class="image-grid-item" onclick="selectSearchImage('${escapeHtml(full)}', this)" title="${escapeHtml(alt)}">
                <img src="${escapeHtml(thumb)}" alt="${escapeHtml(alt)}" loading="lazy">
                <span class="photographer">${escapeHtml(photographer)}</span>
            </div>`;
        }).join('');
